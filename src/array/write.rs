...
pub(crate) mod mapped {
    ...
    impl<'a, T, const N: usize> ArrayRwLockWriteGuard<'a, T, N> {
            ...
            U: ?Sized,
        {
            ...
            unsafe {
                MappedArrayRwLockWriteGuard {
                    ...
                    data: NonNull::from_mut(f(Allocation::get_array_mut_disjoint(orig.0.allocation, orig.0.start))),
                }
            }
        }
        ...
        {
            // SAFETY: All invariants are upheld by construction.
            match f(unsafe { Allocation::get_array_mut_disjoint(orig.0.allocation, orig.0.start) }) {
                Some(data) => {
                    let orig = ManuallyDrop::new(orig);
